...
void ElaPromotionViewPrivate::onPromotionCardClicked(ElaPromotionCard *clickedCard) {
    ...
    for (int i = 0; i < _promotionCardList.count(); i++) {
        ...
        if (i == 0) {
            //最左侧卡片
            QRect targetGeometry(-_pCardCollapseWidth + _leftPadding, 0, _pCardCollapseWidth,
                                 q->height() - _bottomMargin);
            if (_promotionCardList.count() > 2) {
                if (!isRightToLeft) {
                    geometryAnimation->setKeyValueAt(0.7, QRect(geometry.x() + _pCardCollapseWidth * 0.7 + _cardSpacing,
            ...
            //展开的卡片
            geometryAnimation->setEndValue(QRect(_leftPadding + _cardSpacing, 0, _pCardExpandWidth,
                                                 q->height() - _bottomMargin));
            ...
            if (isRightToLeft && targetGeometry.x() > card->x()) {
                connect(geometryAnimation, &QPropertyAnimation::valueChanged, this, [=]() {
                    if (card->geometry().right() <= 0) {
                        ...
                        geometryAnimation->setKeyValueAt(
                            timeRatio, QRect(
                                _promotionCardList[_getAdjacentIndex(Qt::RightToLeft, originIndex)]->geometry().right()
                                + _pCardCollapseWidth * (1 - timeRatio), 0, _pCardCollapseWidth,
                                q->height() - _bottomMargin));
                        geometryAnimation->setEndValue(targetGeometry);
                        geometryAnimation->resume();
                    }
                });
                geometryAnimation->setEndValue(QRect(-_pCardCollapseWidth * 2 + _leftPadding, 0, _pCardCollapseWidth,
                                                     q->height() - _bottomMargin));
            } else {
                geometryAnimation->setEndValue(targetGeometry);
            }
...
void ElaPromotionViewPrivate::_updatePromotionCardGeometry() {
    Q_Q(ElaPromotionView);
    if (_promotionCardList.count() == 0)return;
    if (_promotionCardList.count() < 3) {
        if (_promotionCardList.count() == 1) {
            ...
            _promotionCardList[1]->setGeometry(QRect(lastGeometry.right() + _cardSpacing, lastGeometry.y(),
                                                     _pCardCollapseWidth, lastGeometry.height()));
        ...
        for (int i = 0; i < _promotionCardList.count(); i++) {
            if (i == 0) {
                ...
                _promotionCardList[i]->setGeometry(QRect(lastGeometry.right() + _cardSpacing, lastGeometry.y(),
                                                         _pCardCollapseWidth, lastGeometry.height()));
                _promotionCardList[i]->setHorizontalCardPixmapRatio(0.5);
            }
        }
