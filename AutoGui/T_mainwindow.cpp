    ...
    : ElaWindow(parent)
{
    ...
    connect(this, &T_mainwindow::closeButtonClicked, this, &T_mainwindow::onCloseButtonClicked);
...
void T_mainwindow::onCloseButtonClicked()
{
    ElaContentDialog dialog(this);
    connect(&dialog, &ElaContentDialog::rightButtonClicked, this, &T_mainwindow::closeWindow);
    connect(&dialog, &ElaContentDialog::middleButtonClicked, this, &T_mainwindow::showMinimized);
    dialog.exec();
}

void T_mainwindow::initWindow()
{
    ...
    setNavigationBarDisplayMode(ElaNavigationType::Compact);//左栏目是否收缩
    //setWindowButtonFlag(ElaAppBarType::MinimizeButtonHint, false);//最下化按钮
    //setWindowButtonFlag(ElaAppBarType::MaximizeButtonHint, false);//最大化按钮
    //setWindowButtonFlag(ElaAppBarType::CloseButtonHint, false);//关闭按钮
    //setWindowButtonFlag(ElaAppBarType::ThemeChangeButtonHint, false);//主题按钮
    //setWindowButtonFlag(ElaAppBarType::StayTopButtonHint, false);//前置按钮
    // 
...
void T_mainwindow::initEdgeLayout()
{
    //菜单栏
    ...
    //customWidget->setStyleSheet("border: 1px solid red;");
    //setStyleSheet("border: 1px solid red;");
    //customWidget->setWindowFlags(Qt::FramelessWindowHint);
    //customWidget->setWindowTitle("");
    ...
    updateDockWidget = new ElaDockWidget("更新内容", this);
...
void T_mainwindow::initContent()
{
    _homePage = new T_Home(this);
    
#ifdef Q_OS_WIN
    _elaScreenPage = new T_ElaScreen(nullptr);
    _elaScreenPage->setStyleSheet("border: 1px solid red;");
    //_elaScreenPage->setWindowTitle("");
#endif
...
#ifdef Q_OS_WIN
    addPageNode("DXGI截图", _elaScreenPage,ElaIconType::TvMusic);
    ...
    connect(this, &ElaWindow::navigationNodeClicked, this, [=](ElaNavigationType::NavigationNodeType nodeType, QString nodeKey) {
        if (_aboutKey == nodeKey)
        {
            ...
            aboutPage->show();
        }else if(_elaScreenPage->property("ElaPageKey").toString()==nodeKey){
            if(!paddleDockWidget){
                paddleDockWidget = new ElaDockWidget("目标检测", this);
                paddleDockWidget->setWidget(new DxgiWidget(this,_elaScreenPage));
                this->addDockWidget(Qt::RightDockWidgetArea, paddleDockWidget);
                resizeDocks({paddleDockWidget}, {200}, Qt::Horizontal);
            }else{
                paddleDockWidget->show();
            }
            logDockWidget->hide();
            updateDockWidget->hide();
        }else{
            
            paddleDockWidget->hide();
            logDockWidget->show();
    ...
    connect(this, &T_mainwindow::userInfoCardClicked, this, [=]() { this->navigation(_homePage->property("ElaPageKey").toString()); });
